...
local idle_state = setmetatable({},{__mode = "k"})

function performTask.idle(npc,task)
	if isPedInVehicle(npc) then return true end
	local duration = task[2]
	local block,anim = task[3],task[4]
	local tasknum = getElementData(npc,"npc_hlc:thistask")

	local state = idle_state[npc]
	if not state or state.task ~= tasknum then
		state = {task = tasknum,tick = getTickCount()}
		idle_state[npc] = state
		stopNPCWalkingActions(npc)
		if block and anim then
			setPedAnimation(npc,block,anim,-1,true,false,false,false)
		else
	...
	if getTickCount()-state.tick >= duration then
		idle_state[npc] = nil
		setPedAnimation(npc,false)
		return true
	end
