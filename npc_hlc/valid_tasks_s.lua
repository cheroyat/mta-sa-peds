...
function taskValid.idle(task)
	local duration = tonumber(task[2])
	if not duration or duration <= 0 then return false end
	local block,anim = task[3],task[4]
	if block and type(block) ~= "string" then return false end
	if anim and type(anim) ~= "string" then return false end
	return true
end